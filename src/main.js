...
const createFilter = (filterId) => {
  const filterLabelGenerate = () => {
    ...
    filterLabelElement.insertAdjacentHTML(`beforeend`, `${filterId}
      <span class="filter__${filterIdLowerCase}-count">${getRandomInt(FILTER_COUNT_MIN, FILTER_COUNT_MAX)}</span>`);
  ...
  filterContainerElement.insertAdjacentHTML(`beforeend`, `<input
    ...
    type='radio'
    id='filter__${filterIdLowerCase}'
    name='filter'
    checked>
  `);
  filterContainerElement.appendChild(filterLabelGenerate());
};
